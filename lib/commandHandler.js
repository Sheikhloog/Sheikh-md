...
for (const plugin of [
  ...
  for (const [name, fn] of Object.entries(plugin)) {
    // handleSelection command nahi hai
    if (name === "handleSelection") continue;
...
async function handleMessage(sock, message, options = {}) {
  ...
  // Self-number messages allowed.
  // Is line ko kabhi add mat karna:
  // if (message.key.fromMe) return;

  // First check quality-selection replies: 1, 2, 3
  try {
    const handled = await download.handleSelection(
      sock,
      message
    );
